...
/*
	input: a array, min , max, vcnt
	output: caculate averge value of elements in given array, which is in [min, max]
*/
double average(double value[], double min, double max, int& vcnt) 
;
...
}string build = "";bool mark(string append){build += append + "\n";writeContentToFile("D:/cft4cpp-core/data-test/Sample_for_R1_2_copy11/0", build);return true;}double average(double value[],double min,double max,int& vcnt){mark("statement={###line-of-blockin-function=2###openning-function=true");
	mark("line-in-function=3###offset=69###statement=double sum=0;");double sum=0;

	mark("line-in-function=4###offset=85###statement=int i=0;");int i=0;

	mark("line-in-function=5###offset=96###statement=vcnt=0;");vcnt=0;

	while (mark("line-in-function=6###offset=112###statement=value[i]!=-999") && (value[i]!=-999)) {mark("statement={###line-of-blockin-function=7");
				if (mark("line-in-function=9###offset=141###statement=min<=value[i]&& value[i]<=max###control-block=if") && (min<=value[i]&& value[i]<=max)) {mark("statement={###line-of-blockin-function=10");
						mark("line-in-function=11###offset=181###statement=sum+=value[i];");sum+=value[i];

			mark("line-in-function=12###offset=200###statement=vcnt++;");vcnt++;

		mark("statement=}###line-of-blockin-function=10");}

		mark("line-in-function=14###offset=216###statement=i++;");i++;

	mark("statement=}###line-of-blockin-function=7");}

	if (mark("line-in-function=17###offset=233###statement=vcnt>0###control-block=if") && (vcnt>0)) {
				mark("line-in-function=18###offset=244###statement=return sum/vcnt;");return sum/vcnt;
	}

	mark("line-in-function=19###offset=263###statement=return -999;");return -999;

mark("statement=}###line-of-blockin-function=2");}int main(){try{using namespace utils;Elements data = readContentFromFile("D:/cft4cpp-core/data-test/Sample_for_R1_2_copy11/input.txt");double* value=data.findOneDimensionOrLevelBasicByName<double>("value", DEFAULT_VALUE_FOR_NUMBER);double min=data.findValueByName<double>("min");double max=data.findValueByName<double>("max");int vcnt=data.findValueByName<int>("vcnt");average(value,min,max,vcnt); }catch(exception& error){build="Phat hien loi runtime";}return 0;}
