...
#include <string>#include <iostream>#include <fstream>#include <stdlib.h>using namespace std;void writeContentToFile(char* path, string content){ofstream myfile;myfile.open(path);myfile << content;myfile.close();}string build = "";bool mark(string append){build += append + "\n";writeContentToFile("D:/cft4cpp-core/data-test/tsdv/Sample_for_R1_2_copy11111111/2.txt", build);return true;}int PlusTest(int x,int y){mark("statement={###line-of-blockin-function=1###openning-function=true");
	if (mark("line-in-function=4###offset=55###statement=321 + y * x *1234> 1000###control-block=if") && (321 + y * x *1234> 1000)) {
				mark("line-in-function=5###offset=83###statement=return 1;");return 1;
	}
	else {
				mark("line-in-function=7###offset=103###statement=return 0;");return 0;
	}

mark("statement=}###line-of-blockin-function=1");}int main(){try{int x=0;int y=1;PlusTest(x,y); }catch(exception& error){build="Phat hien loi runtime";}return 0;}
