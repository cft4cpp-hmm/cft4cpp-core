...
#include <string>#include <iostream>#include <fstream>#include <stdlib.h>using namespace std;void writeContentToFile(char* path, string content){ofstream myfile;myfile.open(path);myfile << content;myfile.close();}string build = "";bool mark(string append){build += append + "\n";writeContentToFile("D:/cft4cpp-core/data-test/tsdv/Sample_for_R1_2_copy1/2.txt", build);return true;}int divisionTest(int a,int b){mark("statement={###line-of-blockin-function=1###openning-function=true");
	mark("line-in-function=2###offset=33###statement=int x = a+b;");int x = a+b;

	mark("line-in-function=3###offset=48###statement=int y = x +10;");int y = x +10;

	if (mark("line-in-function=4###offset=68###statement=x<10###control-block=if") && (x<10)) {mark("statement={###line-of-blockin-function=4");
				mark("line-in-function=5###offset=78###statement=return 1/x;");return 1/x;

	mark("statement=}###line-of-blockin-function=4");}
	else {
				mark("line-in-function=7###offset=101###statement=return 1/y;");return 1/y;
	}

mark("statement=}###line-of-blockin-function=1");}int main(){try{int a=-384;int b=737;divisionTest(a,b); }catch(exception& error){build="Phat hien loi runtime";}return 0;}
